...
dir_img = Path('./data/imgs/clean')
dir_flow = Path('./data/flow/')
...
def train_model(
        ...
        img_scale: float = 1.0,  # modify 2: original = .5
        amp: bool = False,
    ...
    # 1. Create dataset
    dataset = BasicDataset(dir_img, dir_flow, img_scale)
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                experiment.log({
                    ...
                    '1px accuracy': metrics['1px'],
                    '3px accuracy': metrics['3px'],
                    '5px accuracy': metrics['5px'],
                    'step': global_step,
                    'epoch': epoch
                })
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        ...
                        logging.info('Validation epe score: {}'.format(val_score))
                        try:
                            experiment.log({
                                'learning rate': optimizer.param_groups[0]['lr'],
                                'validation epe score': val_score,
...
def get_args():
    ...
    parser.add_argument('--scale', '-s', type=float, default=1.0, help='Downscaling factor of the images') # modify 2: change scale 0.5 -> 1.0
